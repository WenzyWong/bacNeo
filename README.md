...
```
Usage: bacNeo [ --download-db -t THREADS | --extract-matrix -d BACC_OUT_DIR -l TAXONOMY -m METHOD ]
...
```
Usage: bacc [ -1 FQ1 ] [ -2 FQ2 ] [-m OMICS] [ -r REF ] [ -o OUT ] [ -t THREADS ] [-l TAXONOMY] [ -c ]
    ...
    Make sure that you have already run bacNeo --download-db previously to generate required databases.
